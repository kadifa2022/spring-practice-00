...
import com.cydeo.entity_model.Movie;
...
public interface MovieRepository extends JpaRepository <Movie, Long>{
    ...
    //Write a derived query to read a movie with the name
    Optional<Movie> readByName(String name);

    //Write a derived query to list all movies between a range of price
    List<Movie> findAllByPriceBetween(BigDecimal price1, BigDecimal price2);

    //Write a derived query to list all movies where duration exists in the specific list of duration
    List<Movie> findAllByDurationIn(List<Integer> durations); // list of durations
    ...
    //Write a native query that return a movie by name
    @Query(value = "SELECT * FROM movie WHERE  name =?1", nativeQuery = true)
    List<Movie> retrieveByName(@Param("name") String name);// can be Optional<Movie> retrieveByName(@Param("name") String name)
