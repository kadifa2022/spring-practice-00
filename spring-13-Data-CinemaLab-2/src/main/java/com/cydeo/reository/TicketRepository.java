...
public interface TicketRepository extends JpaRepository<Ticket, Long> {
    ...
    //Write a derived query to count how many tickets are sold for specific movie
    Integer countAllByMovieCinemaMovieName(String name);

    //Write a derived query to list all tickets between a range of dates
    List<Ticket> findAllByDateTimeBetween(LocalDateTime dateTimeStart, LocalDateTime dateTimeEnd);
    ...
    List<Ticket> fetchAllTicketsBetweenRangeOfDateTimes(@Param("dateTimeStart") LocalDateTime dateTimeStart,
    ...
    @Query(value = "SELECT COUNT(*) FROM ticket WHERE user_account_id = ?1 AND date_time BETWEEN ?2, ?3", nativeQuery = true)
    Integer countTicketByUserInDateRange(@Param("userId") Long userId, @Param("dateTimeStart") LocalDateTime dateTimeStart,
                                         @Param("dateTimeEnd") LocalDateTime dateTimeEnd);
    ...
    // containable in username or account name or movie name

    @Query(value = "SELECT * FROM ticket t JOIN user_account ua ON t.user_account_id = ua.id " +
            "JOIN account_detail ad ON ad.id = ua.account_details_id " +
