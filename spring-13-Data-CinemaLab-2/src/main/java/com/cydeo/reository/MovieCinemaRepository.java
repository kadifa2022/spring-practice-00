...
public interface MovieCinemaRepository extends JpaRepository <MovieCinema, Long> {
    ...
    Optional<MovieCinema> findById(Long id ); // we already have findById() inside JpaRepository just the sample
    ...
    List<MovieCinema> findFirst3ByOrderByMoviePriceDesc(); // if we're sorting we don't pass parameter

    //Write a derived query to list all movie cinemas that contain as specific movie name
    ...
    List<MovieCinema> fetchAllWithHigherThanSpecificData(@Param("dateTime") LocalDateTime dateTime);
